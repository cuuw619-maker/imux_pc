...
bool HandleEditorKey(HWND hwnd, WPARAM wp, LPARAM lp) {
    ...
    if (wp == VK_OEM_4 || wp == VK_OEM_6) {
        std::vector<const EditorItem*> visible;
        for (const auto& item : g_editorItems) {
            if (EditorItemVisibleOnPage(item)) visible.push_back(&item);
        }
        if (!visible.empty()) {
            size_t index = 0;
            for (size_t i = 0; i < visible.size(); ++i) {
                if (visible[i]->id == g_editorSelected) { index = i; break; }
            }
            index = (wp == VK_OEM_4)
                ? (index == 0 ? visible.size() - 1 : index - 1)
                : (index + 1) % visible.size();
            g_editorSelected = visible[index]->id;
            InvalidateRect(hwnd, nullptr, FALSE);
        }
        return true;
    }
...
void BeginEditorDrag#define NOMINMAX
#include <windows.h>
#include <windowsx.h>
#include <d2d1.h>
#include <dwrite.h>
